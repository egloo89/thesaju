...
const fortuneTypePrompts: Record<string, string> = {
  ...
  children: '자녀운',
  earlyLife: '초년운 (0~20세, 유년·학창기)',
  youth: '청년운 (20~40세, 사회진출·결혼기)',
  middleAge: '중년운 (40~60세, 성취·안정기)',
  latterLife: '말년운 (60세 이후 노년의 흐름)',
};
...
function buildDemoTexts(saju: SajuResult): Record<string, string> {
  ...
  return {
    ...
    earlyLife: [
      `초년운(0~20세)에서 당신은 ${firstDaewun ? `${firstDaewun.age}세 무렵 ${firstDaewun.stemHanja}${firstDaewun.branchHanja} 대운에 들어서기 전까지, 타고난 ${el}의 기질을 바탕으로 성장합니다.` : `타고난 ${el}의 기질을 바탕으로 성장합니다.`} 가정 환경과 부모의 사랑이 인격 형성의 토대가 되는 시기입니다.`,
      `이 시기에는 ${strongest.name} 기운이 강해 ${strongest.name === '화' ? '활발하고 표현력이 풍부한 아이' : strongest.name === '금' ? '야무지고 똑부러지는 아이' : strongest.name === '토' ? '듬직하고 신중한 아이' : strongest.name === '목' ? '호기심 많고 적극적인 아이' : '생각이 깊고 차분한 아이'}로 자라는 경향이 있습니다. 부족한 ${weakest.name} 기운을 보완하는 교육·환경을 마련해 주면 재능이 고루 꽃핍니다.`,
      `학업의 기초를 다지고 좋은 습관을 들이기에 중요한 때입니다. 지나친 강요보다 아이의 기질을 존중하며 자존감을 키워주는 것이 평생의 자산이 됩니다.`,
      `종합하면 초년은 평생을 지탱할 뿌리를 내리는 시기입니다. 이 시기에 형성된 가치관과 습관이 이후 인생 전반의 방향을 결정하게 됩니다.`,
    ].join('\n\n'),

    youth: [
      `청년운(20~40세)에서 당신은 사회로 진출하며 본격적으로 자신의 길을 개척합니다. ${midDaewun ? `${saju.daewun[1] ? saju.daewun[1].age : 20}세 전후의 대운 흐름을 타고` : ''} 학업·취업·연애·결혼 등 인생의 중요한 선택들이 집중되는 시기입니다.`,
      `${strongest.name} 기운이 강한 당신은 이 시기에 ${strongest.name === '화' || strongest.name === '목' ? '의욕적으로 도전하며 빠르게 성장하지만, 조급함으로 시행착오를 겪을 수도' : '신중하게 기반을 다지며 안정적으로 나아가지만, 때로 기회를 놓칠까 망설일 수도'} 있습니다. 부족한 ${weakest.name} 기운을 보완하는 전략이 도약의 열쇠가 됩니다.`,
      `직업과 배우자를 정하는 결정적 시기인 만큼, 자신의 사주 강점에 맞는 분야를 선택하고 보완적 인연을 만나는 것이 중요합니다. 실패를 두려워하기보다 경험으로 삼아 실력을 축적하십시오.`,
      `종합하면 청년기는 인생의 기초 공사를 완성하는 시기입니다. 이 시기의 노력과 선택이 중년 이후의 성취를 결정하니, 길게 보며 자신에게 투자하십시오.`,
    ].join('\n\n'),

    middleAge: [
      `중년운(40~60세)에서 당신은 그동안 쌓아온 실력과 인맥이 결실로 이어지는 황금기를 맞이합니다. ${midDaewun ? `특히 ${midDaewun.age}세 전후 ${midDaewun.stemHanja}${midDaewun.branchHanja} 대운에서 사회적 성취와 안정이 무르익습니다.` : '사회적 성취와 안정이 무르익는 시기입니다.'}`,
      `${el} 일간의 당신은 중년에 이르러 ${saju.dayMasterOhaeng === '금' || saju.dayMasterOhaeng === '토' ? '쌓아온 신뢰와 전문성으로 확고한 지위를 다지며' : '풍부한 경험과 유연함으로 새로운 도약을 이루며'}, 재물과 명예가 함께 성장하는 흐름입니다. 다만 과다한 ${strongest.name} 기운으로 인한 무리는 건강과 관계에 부담이 될 수 있으니 균형이 필요합니다.`,
      `이 시기는 후배를 키우고 사회에 환원하며 인생의 의미를 넓혀가는 때이기도 합니다. 자녀의 성장, 부모의 노후 등 가정의 책임도 커지니, 일과 삶의 조화를 의식적으로 관리하십시오.`,
      `종합하면 중년기는 당신 인생의 정점이자 결실의 시기입니다. 젊을 때 뿌린 씨앗이 큰 나무로 자라는 때이니, 자신감을 갖고 그동안의 노력을 마음껏 펼치십시오.`,
    ].join('\n\n'),
